...
Payment core (D-6/D-7) · merge/unmerge/transfer V2 contracts · D-2/D-5 location propagation · inventory consume/waste · agg/reconciler trigger · Z-report · auth core · order state machine · **Total SSOT engine (calculate_order_total_v3 + 20260910000004)**.

### AÇIQ (növbəti — `MASTER_FEATURE_MAP.md` §9-a bax, orada tam Wave planı var)
**Addım 2 (Wave A → B):** outbox consumer (P1, 220 event birikib) · QR anon customer access · gift card UI · customer timeline UI · daily operating checklists · waitlist SMS + reservation reminder · cron verification (6 job) · device registry + print routing · loyalty tiers/birthday UI (Q2) · online ordering (customer UI) · kiosk · upsell UI · batch/expiry tracking · table analytics bloku.
...
- **Outbox:** `outbox_events`-a yazılır amma **consumer YOX** (220+ event birikib) — Addım 2 P1; event-ə əsaslanan yeni feature qurmadan əvvəl consumer qurulmalı.
