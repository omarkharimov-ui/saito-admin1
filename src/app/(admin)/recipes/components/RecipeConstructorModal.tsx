...
import { X, Plus, Trash2, Loader2, CookingPot, FlaskConical, Sparkles, Wand2, Layers3, PencilLine, BadgeInfo, CheckCircle2, ArrowRight, CircleAlert } from 'lucide-react';
...
export function RecipeConstructorModal({ isOpen, onClose, onSaved, editProductId }: RecipeConstructorModalProps) {
  ...
  const [manualMode, setManualMode] = useState<'ai' | 'edit'>(editProductId ? 'edit' : 'ai');
  ...
  const workflowModeLabel = manualMode === 'ai' ? 'AI suggestion first' : 'Manual edit mode';
          ...
          >
            ...
            <div className="overflow-y-auto p-5 sm:p-6 lg:p-8 space-y-6">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <button type="button" onClick={() => setManualMode('ai')} className={`rounded-2xl border px-4 py-3 text-left transition-all ${manualMode === 'ai' ? 'border-violet-400/30 bg-violet-400/10' : 'border-white/10 bg-white/[0.03]'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.25em] text-white/35">AI flow</div>
                      <div className="mt-1 text-sm text-white/80">Suggest → review → save</div>
                    </div>
                    <Sparkles size={14} className="text-violet-300" />
                  </div>
                </button>
                <button type="button" onClick={() => setManualMode('edit')} className={`rounded-2xl border px-4 py-3 text-left transition-all ${manualMode === 'edit' ? 'border-gold/30 bg-gold/10' : 'border-white/10 bg-white/[0.03]'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.25em] text-white/35">Manual flow</div>
                      <div className="mt-1 text-sm text-white/80">Direct edit control</div>
                    </div>
                    <PencilLine size={14} className="text-gold" />
                  </div>
                </button>
              </div>
                ...
                <>
                  <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 space-y-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="hidden sm:flex items-center gap-2 text-white/45 text-[10px] uppercase tracking-[0.25em] font-bold">
                        <BadgeInfo size={11} /> {workflowModeLabel}
                      </div>
                    </div>

                    {aiPreview && (
                      <div className="w-full rounded-2xl border border-violet-500/20 bg-violet-500/8 p-3 text-xs text-white/65">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={aiSuggest} disabled={aiSuggesting || !selectedProduct}
                  ...
                  {manualMode === 'ai' && (
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs text-white/55 flex items-center gap-2">
                      <CircleAlert size={13} className="text-amber-300" />
                      AI-suggested lines appear above. Review the grid below before saving.
                    </div>
                  )}

                  {rows.length === 0 && (
                    <div className="text-center py-8 text-white/20 text-xs rounded-3xl border border-dashed border-white/10 bg-white/[0.02]">
