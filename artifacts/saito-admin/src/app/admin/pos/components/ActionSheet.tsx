...
const fastTransition = { type: "spring", stiffness: 380, damping: 32, mass: 1 } as const;
...
export function ActionSheet({ 
  ...
  return (
    <>
      {/* Backdrop - Only for non-bar views */}
      <AnimatePresence>
        {(currentView === 'actions' || currentView === 'split') && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-black/10 dark:bg-black/40 backdrop-blur-sm"
            onClick={onClose}
          />
        )}
      </AnimatePresence>
      ...
      <div className="fixed bottom-8 left-0 right-0 z-50 flex justify-center pointer-events-none">
        <AnimatePresence mode="wait">
          ...
          {currentView === 'actions' && (
            <motion.div
              key="actions-panel"
              layoutId="pos-bottom-panel"
              initial={{ opacity: 0, y: 100 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 100 }}
              transition={fastTransition}
              className={`relative pointer-events-auto overflow-hidden shadow-[0_25px_50px_rgba(0,0,0,0.3)] backdrop-blur-3xl border rounded-[2.5rem] p-7 w-[90%] max-w-md ${lightMode ? 'bg-white border-zinc-200' : 'bg-zinc-900/95 border-white/10'}`}
            >
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                ...
                <button onClick={onClose} className="w-full mt-5 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest bg-[var(--theme-surface-soft)] hover:brightness-95">Bağla</button>
              </motion.div>
            </motion.div>
          )}
          ...
          {(currentView === 'merge' || currentView === 'transfer') && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 100 }}
              transition={fastTransition}
              className={`relative pointer-events-auto overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.2)] backdrop-blur-3xl border rounded-full px-6 py-3 min-w-[320px] max-w-md ${lightMode ? 'bg-white border-zinc-200' : 'bg-zinc-900/95 border-white/10'}`}
            >
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-5 w-full">
