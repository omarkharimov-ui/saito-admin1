...
--    instead of DELETE. Audit trail preserved.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- READY/SERVED items cannot be voided — use waste instead
    ...
    -- SOFT-DELETE: kitchen_status → 'voided' (audit trail preserved)
    ...
    ELSE
      ...
      UPDATE public.order_items SET
        quantity = v_new_qty,
        total_price = COALESCE(unit_price, 0) * v_new_qty
      WHERE id = v_oi.id;
    END IF;
...
-- ============================================================
-- 2. record_item_waste: reason validation + soft-delete
-- ============================================================
...
BEGIN
  ...
  -- Record waste in cancelled_orders
  ...
  -- SOFT-DELETE: kitchen_status → 'voided'
...
-- ============================================================
-- 3. mark_item_ready_atomic — PREPARING → READY atomically
--    state check + transition + stock consume + audit
--    If stock fails, item stays PREPARING
-- ============================================================

CREATE OR REPLACE FUNCTION public.mark_item_ready_atomic(
  p_order_id    uuid,
  p_item_ids    uuid[] DEFAULT NULL,  -- NULL = all items in order
  p_performed_by uuid DEFAULT NULL
)
  RETURNS jsonb
  LANGUAGE plpgsql
...
BEGIN
  ...
  -- Build list of item IDs to process
  ...
  LOOP
    SELECT * INTO v_oi FROM public.order_items WHERE id = v_item_id AND order_id = p_order_id;
    IF NOT FOUND THEN
      v_skipped := v_skipped + 1;
      CONTINUE;
    END IF;

    -- Already ready/served — skip
    IF v_oi.kitchen_status IN ('ready', 'completed', 'served') THEN
      v_already_ready := v_already_ready + 1;
      CONTINUE;
    END IF;

    -- Only PREPARING/ACCEPTED/SENT items can transition to READY
    IF v_oi.kitchen_status NOT IN ('pending', 'accepted', 'sent', 'preparing') THEN
      v_skipped := v_skipped + 1;
      CONTINUE;
    END IF;

    -- ATOMIC: transition state + consume stock in one transaction
    BEGIN
      -- Transition to READY
      UPDATE public.order_items
      SET kitchen_status = 'ready', updated_at = v_now
      WHERE id = v_item_id;

      -- Consume stock (via inventory_logs — trigger updates current_stock)
      PERFORM public.consume_stock_for_item(v_item_id, p_order_id, v_oi.product_id, v_oi.quantity);

      v_marked := v_marked + 1;

    EXCEPTION WHEN OTHERS THEN
      -- Stock consumption failed — rollback state transition
      UPDATE public.order_items
      SET kitchen_status = v_oi.kitchen_status
      WHERE id = v_item_id;
  ...
  -- Update order kitchen_status if all items ready
  IF NOT EXISTS (
  ...
  PERFORM public.log_audit(
    ...
    p_performed_by, v_performer_name, NULL,
    jsonb_build_object(
      'marked_ready', v_marked,
      'skipped', v_skipped,
      'stock_failed', v_stock_failed,
      'already_ready', v_already_ready,
...
-- ============================================================
-- 4. consume_stock_for_item — single item stock deduction
--    Called by mark_item_ready_atomic for each item
-- ============================================================

CREATE OR REPLACE FUNCTION public.consume_stock_for_item(
  p_order_item_id uuid,
  p_order_id      uuid,
  p_product_id    uuid,
  p_quantity      int
...
DECLARE
  ...
  v_log jsonb;
BEGIN
  -- Check idempotency — already consumed?
  IF EXISTS (
    SELECT 1 FROM public.inventory_logs
    WHERE type = 'order_consumption' AND order_item_id = p_order_item_id
  ) THEN
    RETURN;
  END IF;
  ...
  -- Ready product: direct_ingredient_id
  IF v_prod.is_ready_product AND v_prod.direct_ingredient_id IS NOT NULL THEN
    INSERT INTO public.inventory_logs (ingredient_id, type, quantity, order_id, order_item_id, item_quantity, reference_type, reference_id, reason)
    VALUES (v_prod.direct_ingredient_id, 'order_consumption', p_quantity, p_order_id, p_order_item_id, p_quantity, 'order', p_order_id, 'Hazır məhsul satışı');
    RETURN;
  END IF;

  -- Recipe product: consume each ingredient
  FOR v_rec IN
  ...
  LOOP
    DECLARE
      ...
      v_val NUMERIC;
    BEGIN
      v_raw := COALESCE(v_rec.quantity_brutto, v_rec.quantity_required);
      IF v_rec.unit = 'gram' THEN
        v_val := v_raw;
      ELSIF v_rec.unit = 'ml' THEN
        v_val := v_raw;
      ELSE
        v_val := v_raw;
      END IF;
      ...
      INSERT INTO public.inventory_logs (ingredient_id, type, quantity, order_id, order_item_id, item_quantity, reference_type, reference_id, reason)
      VALUES (v_rec.ingredient_id, 'order_consumption', v_val * p_quantity, p_order_id, p_order_item_id, p_quantity, 'order', p_order_id, 'Reseptli satış');
    END;
  END LOOP;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.consume_stock_for_item(uuid, uuid, uuid, int) TO service_role;
REVOKE ALL ON FUNCTION public.consume_stock_for_item(uuid, uuid, uuid, int) FROM anon;
REVOKE ALL ON FUNCTION public.consume_stock_for_item(uuid, uuid, uuid, int) FROM authenticated;


-- ============================================================
-- 5. dismiss_table_state_aware: soft-delete for all states
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- DRAFT/SENT/PREPARING → soft-void (kitchen_status='voided')
      ...
      -- READY/SERVED/COMPLETED → waste (stock already consumed)
  ...
  -- PAID orders: skip (require refund first)
  FOR v_order IN
    SELECT id FROM public.orders
    WHERE table_number = p_table_number AND status = 'paid'
  LOOP
