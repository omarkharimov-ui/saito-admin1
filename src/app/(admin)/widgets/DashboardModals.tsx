  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between text-sm">
        <span className={`font-serif ${lightMode ? 'text-gray-500' : 'text-white/60'}`}>{start}</span>
        <span className="text-gold font-serif text-lg">—</span>
        <span className={`font-serif ${lightMode ? 'text-gray-500' : 'text-white/60'}`}>{end}</span>
      </div>
      <div ref={trackRef} className={`relative h-12 rounded-2xl overflow-hidden ${lightMode ? 'bg-gray-50' : 'bg-white/[0.03]'}`}>
        {/* Filled track */}
        <div 
...
const GoldenSlider = ({ value, onChange }: { value: number; onChange: (v: number) => void }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div 
        ref={trackRef}
        className={`relative h-4 rounded-full cursor-pointer ${lightMode ? 'bg-gray-200' : 'bg-white/10'}`}
      ...
      <div className={`flex justify-between text-[10px] uppercase tracking-wider ${lightMode ? 'text-gray-400' : 'text-white/30'}`}>
        <span>5%</span>
        <span>50%</span>
      </div>
    </div>
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className={`absolute bottom-0 left-0 right-0 bg-[#0a0a0a]/90 backdrop-blur-2xl border-t rounded-t-3xl overflow-hidden ${lightMode ? 'border-gray-200' : 'border-white/[0.08]'}`}
          ...
          >
            ...
            <div className="sticky top-0 z-10 px-5 pt-6 pb-4 bg-gradient-to-b from-[#0a0a0a] to-transparent">
              <button
                onClick={onClose}
                className={`absolute top-5 left-5 w-10 h-10 flex items-center justify-center rounded-full transition-all ${lightMode ? 'bg-gray-100 text-gray-500 hover:text-gray-900' : 'bg-white/[0.05] text-white/60 hover:text-white'}`}
              >
                <ChevronLeft size={22} />
              </button>
              <div className="text-center pt-2">
                ...
                <h2 className={`text-2xl font-serif ${lightMode ? 'text-gray-900' : 'text-white'}`}>{t('sensei_happy_hour')}</h2>
                <p className={`text-[10px] uppercase tracking-[0.3em] mt-1 ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>Endirim vaxti</p>
              </div>
            </div>
            
            <form noValidate onSubmit={onSubmit} className="px-5 pb-32 overflow-y-auto h-[calc(85vh-140px)]">
              {/* Floating Card - Glassmorphism */}
              <div className={`mb-6 p-5 rounded-2xl backdrop-blur-xl border ${lightMode ? 'bg-gray-50 border-gray-200' : 'bg-white/[0.03] border-white/[0.06]'}`}>
                {/* Time Range */}
                <div className="mb-6">
                  <label className={`text-[10px] uppercase tracking-[0.2em] mb-4 block ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>Vaxt araligi</label>
                  <DualTimeSlider 
                ...
                {/* Discount */}
                <div>
                  <label className={`text-[10px] uppercase tracking-[0.2em] mb-3 block ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>Endirim faizi</label>
                  <GoldenSlider 
              ...
              <div className="space-y-4">
                <label className={`text-[10px] uppercase tracking-[0.2em] ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>{t('select_product')}</label>
                <div className="grid grid-cols-2 gap-3">
                  {products.map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        {/* Image */}
                        <div className={`aspect-square ${lightMode ? 'bg-gray-50' : 'bg-white/[0.03]'}`}>
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/90 to-transparent">
                          ...
                          <p className={`text-[10px] mt-0.5 ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>ℼ{p.price}</p>
                        </div>
                        {/* Selection indicator */}
                        {isSelected && (
  ...
  return (
    <MobileModal open={open} onClose={onClose}>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h3 className={`text-xl font-bold ${lightMode ? 'text-gray-900' : 'text-white'}`}>{t('confirm_delete_title')}</h3>
          <p className={`mt-1 ${lightMode ? 'text-gray-500' : 'text-white/60'}`}>{t('confirm_delete_message')}</p>
        </div>
      </div>
      <div className="flex gap-3">
        <button onClick={onClose} className={`flex-1 py-3 px-4 rounded-xl ${lightMode ? 'bg-gray-100 text-gray-500' : 'bg-white/5 text-white/60'}`}>{t('cancel')}</button>
        <button onClick={onConfirm} disabled={updating} className={`flex-1 py-3 px-4 rounded-xl bg-red-500 disabled:opacity-50 flex items-center justify-center gap-2 ${lightMode ? 'text-gray-900' : 'text-white'}`}>
