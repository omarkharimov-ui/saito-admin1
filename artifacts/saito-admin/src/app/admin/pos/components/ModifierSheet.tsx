...
export function ModifierSheet({ open, productName, productPrice, variants = [], onClose, onConfirm }: ModifierSheetProps) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastExit} className="fixed inset-0 z-[130] bg-black/50 backdrop-blur-sm" onClick={onClose} />
          <motion.div {...slideUp} transition={fastExit} className="fixed bottom-0 inset-x-0 z-[140] max-h-[90vh] overflow-y-auto bg-[var(--theme-surface)] rounded-t-6xl border-t border-white/[0.08] shadow-elevated backdrop-blur-2xl p-6 pb-12" style={{ paddingBottom: bottomOffset > 0 ? bottomOffset + 24 : undefined }}>
            <div className="max-w-2xl mx-auto space-y-8">
              ...
              {variants.length > 0 && (
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-white/30 mb-3">{t('select_option')}</p>
              ...
              {/* Notes */}
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-white/30 mb-3">{t('custom_note')}</p>
              ...
              <div className="flex items-center gap-4 pt-4 border-t border-white/[0.05]">
                <div className="flex-1">
                  <p className="text-xs uppercase font-black tracking-widest text-white/20">{t('grand_total')}</p>
                  <p className="text-2xl font-black text-white tabular-nums">{basePrice.toFixed(2)} ₼</p>
                </div>
                <button onClick={handleConfirm} className="px-10 py-4 rounded-3xl bg-gold text-black font-black text-sm uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all shadow-xl shadow-gold/20 flex items-center gap-2">
                  <Plus size={18} /> t('add')
                </button>
              </div>
            </div>
