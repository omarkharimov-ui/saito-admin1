-- ============================================================================
-- CP-1 (2026-09-20) — get_customer_timeline: expose the three new profile
-- fields (birthday/email/notes) in the 'customer' jsonb. ADDITIVE: signature
-- unchanged, new keys only — existing consumers (UI list stats, timeline)
-- unaffected. Rollback: restore the 09-19 body from
-- supabase/baseline_live_schema_2026-09-14.sql-era W-A3 migration.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_customer_timeline(p_customer_id uuid, p_limit integer)
RETURNS jsonb
LANGUAGE sql STABLE
AS $fn$
  ...
  SELECT jsonb_build_object(
    'customer', (
      SELECT jsonb_build_object('id', c.id, 'name', c.name, 'phone', c.phone,
                                ...
                                'birthday', c.birthday, 'email', c.email, 'notes', c.notes)
