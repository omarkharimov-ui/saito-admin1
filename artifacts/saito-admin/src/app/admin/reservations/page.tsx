...
export default function ReservationsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setFloors(fData || []);
      if (fData?.length) setSelectedFloorId(fData[0].id);
  ...
  const getAIServiceOrder = (items: any[]) => {
    if (!items || items.length === 0) return "Öncədən sifariş daxil edilməyib.";
    ...
    return `AI Guide: Hazırlığa ${sorted[0]?.name} ilə başlayın. Ardıcıllıq: ${sorted.map(i => i.name).join(' → ')}.`;
  };
      ...
      <AnimatePresence>
        {selectedRes && (
          <>
            ...
            <motion.div layoutId={`reserv-${selectedRes.id}`} className={`fixed inset-0 m-auto z-[110] w-[95%] max-w-2xl h-fit max-h-[90vh] overflow-hidden rounded-[3.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.6)] border ${lightMode ? 'bg-white border-zinc-200 text-zinc-900' : 'bg-zinc-900 border-white/10 text-white'}`}>
              <div className="p-10 relative overflow-y-auto max-h-[90vh]">
                       ...
                       <motion.div layout="position">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                             <div onClick={() => setModalView('tables')} className={`p-7 rounded-[2.5rem] border cursor-pointer hover:scale-[1.02] active:scale-95 transition-all shadow-lg ${lightMode ? 'bg-zinc-50 border-zinc-100' : 'bg-white/5 border-white/10'}`}>
                                ...
                                <div className="flex items-center gap-5">
                                   <div className="w-16 h-16 rounded-2xl bg-blue-500 text-white flex items-center justify-center font-black text-2xl shadow-xl shadow-blue-500/20">{selectedTableIds.length || '?'}</div>
                                   <div className="flex flex-col"><span className="text-sm font-black">{selectedTableIds.length ? `${selectedTableIds.length} Masa seçildi` : 'Hələ təyin edilməyib'}</span><span className="text-[10px] opacity-40 font-bold">Rezervi masaya bağla</span></div>
                                </div>
                             </div>
                             <div onClick={() => setModalView('preorder')} className={`p-7 rounded-[2.5rem] border cursor-pointer hover:scale-[1.02] active:scale-95 transition-all shadow-lg ${lightMode ? 'bg-zinc-50 border-zinc-100' : 'bg-white/5 border-white/10'}`}>
                                ...
                                <div className="flex flex-col">
                                   {preOrderItems.length > 0 ? (
                                      <><span className="text-sm font-black">{preOrderItems.length} Məhsul seçilib</span><span className="text-xl font-black text-emerald-500">{preOrderItems.reduce((s, i) => s + i.price, 0).toFixed(2)} ₼</span></>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                             <div className="flex gap-4">
                                <button onClick={handleConfirmReservation} className="flex-[2] py-6 rounded-[2rem] bg-green-500 text-white font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-3">
                                   <CheckCircle size={24} /> Təsdiqlə
                                </button>
                                <button onClick={() => setSelectedRes(null)} className="flex-1 py-6 rounded-[2rem] bg-white/5 border border-white/10 font-black uppercase tracking-widest">Bağla</button>
                             </div>
                             <div className={`p-6 rounded-[2.5rem] flex items-center justify-center gap-4 ${lightMode ? 'bg-zinc-50' : 'bg-white/5'}`}>
                                ...
                                <div className="flex flex-col"><span className="text-[9px] font-black uppercase opacity-40">Bron Vaxtına Qalıb</span><span className="text-2xl font-black">{calculateTimeLeft(selectedRes.time)}</span></div>
                             </div>
                          </div>
                          <div className="mt-8 p-7 rounded-[2.5rem] bg-blue-500/5 border border-blue-500/10 flex items-start gap-5">
                             ...
                             <div className="flex flex-col gap-1"><span className="text-[9px] font-black uppercase text-blue-500 opacity-60">Saito AI Insight</span><p className="text-[14px] font-bold tracking-tight">{getAIServiceOrder(preOrderItems)}</p></div>
                          </div>
                       </motion.div>
                    </motion.div>
                  )}
                  {modalView === 'tables' && (
                    <motion.div key="tables" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex flex-col gap-8">
                       <div className="flex items-center gap-5">
                          ...
                          <div><h3 className="text-3xl font-black tracking-tighter mb-1">Masa Seçimi</h3><p className="text-xs font-black opacity-40 uppercase tracking-widest text-zinc-500">Boş masaları seçin (Birləşdirmək olar)</p></div>
                       </div>
                       <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                          {floors.map(f => (<button key={f.id} onClick={() => setSelectedFloorId(f.id)} className={`px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${selectedFloorId === f.id ? 'bg-blue-500 text-white shadow-lg' : 'bg-white/5 opacity-50'}`}>{f.name}</button>))}
                       </div>
                       <div className="grid grid-cols-4 sm:grid-cols-5 gap-4 max-h-[350px] overflow-y-auto pr-3 custom-scrollbar">
                          {tables.filter(t => t.floor_id === selectedFloorId).map(t => (
                             ...
                             }} className={`aspect-square rounded-[1.8rem] border-3 flex flex-col items-center justify-center transition-all ${selectedTableIds.includes(t.id) ? 'bg-blue-500 border-blue-500 text-white shadow-2xl scale-105' : t.status === 'empty' ? 'bg-white/5 border-white/10' : 'opacity-20 grayscale'}`}>
                       ...
                       <button onClick={() => setModalView('main')} className="w-full py-6 rounded-[2rem] bg-zinc-900 text-white font-black uppercase shadow-xl">Seçimi Saxla</button>
                    </motion.div>
                  )}
                  {modalView === 'preorder' && (
                    <motion.div key="preorder" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex flex-col gap-8">
                       <div className="flex items-center gap-5">
                          <button onClick={() => setModalView('main')} className="p-4 rounded-full bg-white/5 shadow-lg"><ChevronLeft size={28} /></button>
                          <div><h3 className="text-3xl font-black tracking-tighter mb-1">Məhsul Əlavə Et</h3><p className="text-xs font-black opacity-40 uppercase tracking-widest text-zinc-500">Öncədən sifariş yığın</p></div>
                       </div>
                       <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto pr-3 custom-scrollbar">
                          {products.map(p => (
                             <div key={p.id} className="flex items-center justify-between p-5 rounded-3xl bg-white/5 border border-white/5">
                                <div className="flex flex-col gap-0.5"><span className="text-[9px] font-black uppercase text-blue-400">{(p as any).categories?.name}</span><span className="text-base font-bold">{p.name}</span><span className="text-xs font-black opacity-40">{p.price} ₼</span></div>
                                <div className="flex items-center gap-3">
                                   ...
                                   <button onClick={() => setPreOrderItems(prev => [...prev, p])} className="p-2.5 rounded-xl bg-blue-500/10 text-blue-500"><Plus size={20} /></button>
                                </div>
                             </div>
                          ))}
                       </div>
                       <button onClick={() => setModalView('main')} className="w-full py-6 rounded-[2rem] bg-emerald-500 text-white font-black uppercase shadow-xl">Səbəti Saxla ({preOrderItems.reduce((s, i) => s + i.price, 0).toFixed(2)} ₼)</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
