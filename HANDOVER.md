...
Payment core (D-6/D-7) · merge/unmerge/transfer V2 contracts · D-2/D-5 location propagation · inventory consume/waste · agg/reconciler trigger · Z-report · auth core · order state machine · **Total SSOT engine (calculate_order_total_v3 + 20260910000004)** · **Outbox consumer (outbox_pump/outbox_dispatch) + pg_cron scheduler** (A#1) · **🔒 A — AUTHENTICATION/RBAC** (2026-09-11, `d84c20e`, 39/39 regression; açma şərti: real security vuln / data corruption / regression / frozen-contract səhvi — `AUDIT_A_FREEZE.md`) · **🔒 E/S FOUNDATION CONTRACT-ları** (2026-09-11: overtime/timezone/ownership qərarları + S-01 clock-in location resolve + S-02 token-RPC IDOR — `ES_FOUNDATION_CURRENT_STATE.md`; açma şərti: contract səhvi / regression; A dependency toxunulmur) · **🔒 F — FLOOR/TABLES** (2026-09-11, `f29c032`, 35/35 gate, A 39/39 + E/S 54/54; F-01..F-06 + F-10 fixed; açma şərti: real security vuln / data corruption / regression / frozen-contract səhvi — `F_FROZEN.md`) · **🔒 O — ORDERS** (2026-09-11, 8.2→8.11: G1–G7 — atomic transition path + old fn DROPPED, 57+3 RPC service-role-only, QR fixed (public customer path), create/append location-scoped, cancel = orders.cancel atomic (403), reads scoped (0 leakage); O gate **38/38**, A 39/39, E/S 54/54; residual: `idx_orders_active_table` global (F-boundary, deferred) + KDS browser-direct mutations → K; açma şərti: real security vuln / data corruption / regression / frozen-contract səhvi — `O_FROZEN.md`).

### AÇIQ (növbəti — `MASTER_FEATURE_MAP.md` §9-a bax, orada tam Wave planı var)
**🔒 E/S + 🔒 F + 🔒 O = FROZEN** (2026-09-11). **Növbəti: K — KITCHEN/KDS dependency-impact** (O-dan K-ya transfer olunan: browser-direct KDS/admin item mutations = `mark_order_ready`/`prepare_order_items`/`cancel_order_items`/add-qty — server API route adoption tələb edir; O-dan K-a transfer olunan: `idx_orders_active_table` global unique = F-boundary, K multi-location işi ilə birləşdirilə bilər). Ardıcıllıq: **O ✅ → K → P/B → I → R** (dependency-impact, yenidən sıfırdan YOX).
...
- `2026-09-11 — 5.8 — S-08 FIXED (shift↔cash-drawer 1:1 binding, frozen decisions): open_cash_register CRASH fix (loc/org from opener session) + 1:1 atomic bind (UNIQUE index + FOR UPDATE + unique_violation→DRAWER_ALREADY_BOUND); close_cash_register_v2: bound shift auto-close (expected_cash DERIVED from ledger SSOT), cash_drawer_logs audit row, overtime hook. 3 pre-existing bug fix: expected_cash double-count (opening+open-row), staff.role→role_id/roles join, manager_id→approved_by column. close_shift: refuse while drawer open. Migration 20260911000016. Live proof P1-P9+E2E (crash fixed, bind, 1:1 race → 1 wins/1 clean, expected 300 derived NOT 0/400, variance→approval, invalid-mgr rejected), A 39/39, residue 0 — commit bu
- `2026-09-11 — 6.0 — 🔒 E/S FROZEN (final validation gate): `.es-gate.cjs` = **54/54 PASS** (E2E 13 + edge 9 + concurrency 13 + residue 10 + re-audit 9), **A 39/39** (read-only). Gate tapdı + fix etdi 5 latent bug: G-1 start_break_token staff_id NOT NULL (break hamı üçün crash), G-2 /status route unauthenticated (IDOR-read), G-3 currval non-existent seq, clock-out status='OPEN' left (re-clock-in blocked, 7 prod rows repaired), open-shift creation unserialized (advisory lock migs 17+20). Migrations 09–21 (E/S). **S-01→S-09 = FIXED+LIVE (S-07 accepted LOW/defensive).** Open business item: 3 PRE-EXISTING NULL-opened_by drawer sessions (2 OPEN, real money, 2026-07/08) → finance reconciliation, NOT auto-closed. Sənəd: `E_S_FREEZE.md` — commit bu
- `2026-09-11 — F AUDIT (foundation only, NO fixes): `F_FOUNDATION_CURRENT_STATE.md`. 85% rəqəmi atıldı → real audit. F daha tamdır E/S-dən (formal data-driven state machine 25 rule/16 status, FOR UPDATE atomic RPC-lər, RLS ON, outbox, KDS sync, reservation→table). Gaps: **F-01** (table ops-da HEÇ permission/role/manager override YOX — waiter dismiss/merge/cancel edə bilir, money), **F-02** (cross-location table ops bloklunmur — definer RPC RLS bypass, validate_actor yalnız exists+active), **F-03** (GLOBAL UNIQUE table_number = single-location scale blocker), F-04 (68 orphan orders, historical hard-delete), F-05 (3 stale current_order_id → cancelled), F-06 (seats table DEAD, guest_count model), F-07 (16 status, 3 istifadə). 6 business decision FROZE qərara gözləyir — commit bu
- `2026-09-11 — F-01+F-02 FIXED (same batch): table-op permission + location. 8 core fns → token identity (`set_session_staff`) + `has_permission` gate + `has_location_access` (A-frozen) self-enforced in definer RPC; old caller-supplied `p_performed_by` sigs DROPPED. `floor.manage` (NEW minimal perm: manager/owner/admin/superadmin) for clear/dismiss/undo/merge/unmerge/transfer/release; `orders.create` for activate/reserve/seat. 8 routes → requirePermission + p_token. Live proof: waiter→dismiss/merge/transfer=PERMISSION_DENIED, manager(LOC_A)→LOC_B table=FORBIDDEN_LOCATION, own-loc clear=SUCCESS, raw RPC no-token=BLOCKED. A 39/39, residue 0 — commit bu
- `2026-09-11 — F-03 FIXED (multi-location table identity, foundational): `UNIQUE(table_number)` → `UNIQUE(location_id, table_number)`; 2 dependent FK re-key (merged_into_table self-ref + reservation_tables — 12 writer-a toxunulmADIQ, `reservation_tables.location_id` BEFORE INSERT trigger-lə reservation-dan resolve olub, PK qalıb); 7 number-based table-op fn session-location scoped (`v_sess_loc` from `app.current_location_id`); `table_order_contract` view location-scoped; `pos/tables` route (F-11 fix) session active_location filter. DATA: 8 ölü `reservation_tables` sətiri (missing reservation + missing table 200-209, hər iki FK already broken) → `reservation_tables_quarantine_2026_09` (sillik DEYIL, reconciliation). Migration 20260911000023. Live proof: table 1 BOTH locations ✅, dup same-location ❌, LOC_A session floor map = ONLY LOC_A (0 LOC_B leakage), quarantine=8, reservation_tables 9/9 FK-valid, A 39/39 — commit bu
- `2026-09-11 — F-05 FIXED (pointer lifecycle): canonical `current_order_id` recomputed in `sync_table_order_aggregates` (fires on every order change — runtime, NOT cron): latest OPEN order on (table,location) else NULL; guard-safe vs `table_release_guard`; one-time repair. Live proof: 3 stale→0, live order→pointer set, cancel→pointer cleared. Migration 20260911000024 — commit bu
- `2026-09-11 — F-04 FIXED (archive lifecycle): `table_floors.is_archived`+`archived_at`; DELETE always blocked (trg_table_archive_guard, OLD.table_number — no force-delete escape per contract); archived immutable vs use fields (status/reservation/pointer/merge); orders BEFORE INSERT rejects archived tables; `archive_table_atomic` (manager+location). Floor editor `pos/floors` rewritten: DELETE+reINSERT → ARCHIVE+UPSERT (root cause of 68 orphans fixed). Live proof: archive OK+history kept, new-order/use/hard-delete/number-reuse all BLOCKED, un-archive OK. 68 orphans DOCUMENTED (F_TABLE_ORPHAN_RECONCILIATION.md, NOT deleted). Migration 20260911000025. A 39/39 — commit bu
- `2026-09-11 — F-06 FIXED (dead seats model removed): 0-caller sweep (TS 0, DB fn callers 0, REST 0, FKs 0, triggers 0, rows 0) → `DROP FUNCTION upsert_seats/get_seat_totals` + `DROP TABLE seats`. `guest_count INT` canonical qorunur. Migration 20260911000026. Per-seat gələcəkdə separate foundation decision olacaq — commit bu
- `2026-09-11 — 7.0 — 🔒 F FROZEN (final gate): `.f-gate.cjs` (idempotent, START-cleanup) = **35/35 PASS** bir clean run (M1-11 + F10-1..5 + C1-4 + A1-10 + Z1-5), **A 39/39**, **E/S 54/54**. Re-audit: heç köhnə bypass reachable DEYIL (0 caller-identity overload, 0 raw PATCH, 0 bare-requireAuth route). Final re-audit-da `tables/seat` bypass tapılıb + FIXED (host 403, cross-loc 404/untouched, waiter 200) — A10 hard-check. Sənəd: `F_FROZEN.md` — commit bu
- `2026-09-11 — 8.0 — O (Orders) FOUNDATION + DEPENDENCY-IMPACT AUDIT (audit-only, NO fix): `O_FOUNDATION_CURRENT_STATE.md` (60+ probe, real DB + source). Order state-machine VALIDITY = trigger-enforced (solid, unlike pre-F-01 table). CRITICAL 3: **O-01** live transition path (`transition_order_status` via POS `useOrderStateMachine` + `/api/rpc/transition_order_status`) has NO permission/NO location/`performed_by` client-spoofable, while the golden `transition_order_atomic` (token→authorize(loc)→override→audit→outbox) = **DEAD (0 callers)**; **O-02** ~20 order-mutation RPCs expose EXECUTE to `authenticated` role on PostgREST (staff-JWT raw-RPC bypass; anon=N verified); **O-03** QR dine-in order creation 100% dead (blocked by `trg_order_table_location`, 0 qr_orders) + `orders/qr` POST has no requirePermission. **O-04** = the F-03 deferred location-scoping boundary (create_or_append_order/create_order_with_items/walkin_atomic + orders/route.ts POST) STILL OPEN. 🟠 O-05 cancel=raw PATCH TOCTOU no `orders.cancel`; 🟠 O-06 O writers set `table_floors.status` via raw PATCH bypassing table state machine (F boundary — **F stays frozen, do NOT add table trigger in O**). 🟡 O-07 archived-guard bypass on NULL-location insert; 🟡 O-08 `/orders`+`/kitchen/orders` GET no location scope/perm. Reachability: 3 live old paths YES. **GATE G1–G7 gözləyir (user).** A/E/S/F = 🔒 toxunulmadı — commit db6320b`
- `2026-09-11 — 8.12 — 🔒 O FROZEN (final gate): G1–G7 fix batch (8.2→8.11) + final re-audit. **G1**: live transition path → `transition_order_atomic` SUPERSET (auth+perm+loc+override+kitchen+floor sync+audit+outbox); old `transition_order_status` **DROPPED** (2/2 callers rewired); `transition_delivery_status` + `cancel_loss_table` token-first; spoofed identity → 400. **G2**: 57+3 order-mutation RPC **service-role-only** (anon/authenticated REVOKE; live 42501 17/17; browser=anon verified — app Supabase Auth session açmır). **G3**: QR fixed — location/org server-trusted (table row), `/api/orders/qr` public customer path (rate-limited, NOT staff-perm-gated), `order_type` forced. **G4**: create/append/update/addItems/walkin_atomic **active-location scoped** (dead create fns DROPPED, walkin_atomic exactly 1 overload). **G5**: cancel → atomic `orders.cancel` + row lock (TOCTOU closed), waiter=403; create-rollback token-first (system fallback yalnız creator perm-sizdirsə). **G6 NO-OP**: F FROZEN (table trigger əlavə ETMƏDİK). **G7**: reads + `orders.view`/`kitchen.view` + `resolveReadLocationScope` (active→single-loc-with-data→fail-closed; 168 unbound staff kırılmadı). **O gate 38/38** (`.o-gate.cjs`), **A 39/39**, **E/S 54/54**. Reachability: transition_order_status=0, dead creates=0, anon-executable mutation=0, cross-loc leak=0, cancel/pay race 1-winner + no phantom cash, create concurrency 1-active+pointer. Residual (F-boundary, deferred, O-dan yaranmayıb): `idx_orders_active_table` global table_number unique (2 location eyni number-da eyni anda aktiv order saxlaya bilməz) + number-based F-05 triggers dup-number-da ambiguous. K-ya transfer: browser-direct KDS/admin item mutations (`mark_order_ready`/`prepare_order_items`/`cancel_order_items`/add-qty) = ACL-da 42501 (bypass değil, functional no-op) — K audit/fix scope; K onları qəbul edənə qədər "working" deməyəcəyik. Sənəd: `O_FROZEN.md` — commits 27e06ab→6d63f7f`
