-- ============================================================================
-- 20260911000019 — GATE-CRITICAL: clock-out paths must set status='CLOSED'
--
-- ROOT CAUSE (found by the E/S FINAL GATE): clock_out_token and
-- clock_out_atomic_token set closed_at=now() but NOT status. A shift left at
-- status='OPEN' + closed_at set is still counted as OPEN by the
-- trg_shift_no_overlap trigger (it matches `status = 'OPEN'`), so the staff
-- can NEVER clock in again after their first clock-out. 7 such inconsistent
-- rows already existed in production data.
--
-- FIX: close paths set status='CLOSED' (close_shift + close_cash_register_v2
-- already did). One-time data repair for the 7 inconsistent rows.
-- ============================================================================

-- one-time repair
UPDATE public.shifts SET status = 'CLOSED' WHERE closed_at IS NOT NULL AND status <> 'CLOSED';

-- clock_out_token: set status on close
...
BEGIN
  ...
  UPDATE shifts SET closed_at = NOW(), status = 'CLOSED', notes = COALESCE(p_notes, notes)
  WHERE staff_id = p_target_id AND closed_at IS NULL;
...
-- clock_out_atomic_token: set status on close
