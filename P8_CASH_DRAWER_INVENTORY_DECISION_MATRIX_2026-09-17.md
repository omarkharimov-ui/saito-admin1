...
|---|---|---|---|---|
| **D-1** | P1 | **Unscoped drawer selection.** `fn_record_cash_payment`: `WHERE status='open' ORDER BY opened_at DESC LIMIT 1` — no loc/org/shift. LIVE PROOF: 4 orphan open sessions; `a0ff4659` (opened 2026-09-11 by E/S fixture `G2mtwuj3zt_CASH`) holds **138 `payment` rows = 12,900.00 ₼**; `ab751905` 32 rows/736.00; `ca90e940` 15 rows/264.78. `/api/cash-drawer` GET returns the same global newest-open session. | Session pick scoped by `location_id + organization_id` (from the paying order's location), and only `shift_id`-bound-or-location sessions; no matching open session → **no ledger row** (documented skip, no silent global booking). Route GET: location filter + `cash.view`. | P-8 |
...
1. REVOKEs (D-7, D-8, D-15, D-21): anon EXECUTE on all drawer/shift fns; authenticated EXECUTE on
   tokenless writers; anon DML on time_clock_*/daily_reports; test_rls_role grants.
