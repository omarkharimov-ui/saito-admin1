-- ============================================================================
-- 20260911000022 — F-01 + F-02 (ONE BATCH): table-op permission + location
--
-- USER-FROZEN CONTRACTS (F foundation gate, 2026-09-11):
--   F-01 permission model (reuse canonical where possible, 1 new minimal perm):
--     * activate/reserve/seat      -> `orders.create` (held by floor staff)
--     * clear/dismiss/undo/merge/unmerge/transfer/release -> `floor.manage`
--       (NEW, minimal: manager/owner/admin/superadmin — no existing perm =
--       manager-tier floor ops; `orders.cancel` excluded: admin-only, no
--       manager/owner)
--   F-02 location isolation:
--     * identity = p_token via A-frozen set_session_staff -> current_staff_id
--       (caller-supplied performed_by is NO LONGER the authorization source;
--       it becomes audit-only, defaulting to the session actor)
--     * definer RPCs self-enforce has_location_access (A-frozen) — RLS is
--       bypassed by SECURITY DEFINER, so this is the real guard
--     * raw RPC bypass is closed: no valid token -> NO actor -> FORBIDDEN
--   Enforced at RPC (definer) AND route level (route adds requirePermission).
--
-- Also fixes: dismiss_undo_atomic called auth.role() inside SECURITY DEFINER
-- (unreliable in service-role context) -> replaced by token-based session.
-- Old signatures are DROPPED so no unguarded overload remains.
-- GOLDEN RULE 5: auto-commit.
-- ============================================================================

-- activate_table_atomic: token identity + orders.create + location isolation
CREATE OR REPLACE FUNCTION public.activate_table_atomic(p_token text, p_table_id uuid, p_guest_count integer)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF NOT coalesce(has_permission(current_staff_id(), 'orders.create'), false) THEN
...
END;
$$;
-- clear_table_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.clear_table_atomic(p_token text, p_table_number integer, p_performed_by uuid, p_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
-- dismiss_table_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.dismiss_table_atomic(p_token text, p_table_number integer, p_reason text, p_final_status text, p_performed_by uuid, p_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
-- dismiss_undo_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.dismiss_undo_atomic(p_token text, p_table_number integer, p_performed_by uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SELECT location_id INTO v_loc FROM public.table_floors WHERE table_number = p_table_number;
...
END;
$$;
-- merge_tables_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.merge_tables_atomic(p_token text, p_parent_table_number integer, p_child_table_numbers integer[], p_performed_by uuid, p_performed_by_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
-- unmerge_tables_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.unmerge_tables_atomic(p_token text, p_parent_table_number integer, p_child_table_numbers integer[], p_performed_by uuid, p_performed_by_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
-- transfer_table_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.transfer_table_atomic(p_token text, p_from_table integer, p_to_table integer, p_performed_by uuid, p_performed_by_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF (SELECT count(*) FROM public.table_floors
    ...
    RETURN jsonb_build_object('success', false, 'error', 'TABLE_NOT_FOUND');
  END IF;
  SELECT * INTO v_from FROM public.table_floors WHERE table_number = p_from_table;
  SELECT * INTO v_to   FROM public.table_floors WHERE table_number = p_to_table;
...
END;
$$;
-- release_paid_table_atomic: token identity + floor.manage + location isolation
CREATE OR REPLACE FUNCTION public.release_paid_table_atomic(p_token text, p_table_number integer, p_final_status text, p_performed_by uuid, p_terminal_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- F-01: minimal NEW permission `floor.manage` (manager tier) — no existing
-- permission matched (orders.cancel = admin-only, no manager/owner).
INSERT INTO public.permissions (key, description, category, name, code, scope)
VALUES ('floor.manage', 'Manager-level floor/table operations: clear, dismiss, undo, merge, unmerge, transfer, release', 'floor', 'Floor Management', 'floor.manage', 'LOCATION')
ON CONFLICT (key) DO NOTHING;
INSERT INTO public.role_permissions (role_id, permission_key)
SELECT r.id, 'floor.manage' FROM public.roles r
WHERE r.name IN ('manager','owner','admin','superadmin')
ON CONFLICT DO NOTHING;

-- DROP old (unguarded, caller-supplied identity) signatures so no unguarded
-- overload remains. (New token-first overloads are the only path.)
DROP FUNCTION IF EXISTS public.activate_table_atomic(uuid, integer);
DROP FUNCTION IF EXISTS public.clear_table_atomic(integer, uuid, text);
DROP FUNCTION IF EXISTS public.dismiss_table_atomic(integer, text, text, uuid, text);
DROP FUNCTION IF EXISTS public.dismiss_undo_atomic(integer, uuid);
DROP FUNCTION IF EXISTS public.merge_tables_atomic(integer, integer[], uuid, text);
DROP FUNCTION IF EXISTS public.unmerge_tables_atomic(integer, integer[], uuid, text);
DROP FUNCTION IF EXISTS public.transfer_table_atomic(integer, integer, uuid, text);
DROP FUNCTION IF EXISTS public.release_paid_table_atomic(integer, text, uuid, text);

-- execute grants (service role = the route's RPC caller)
GRANT EXECUTE ON FUNCTION public.activate_table_atomic(text, uuid, integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.clear_table_atomic(text, integer, uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.dismiss_table_atomic(text, integer, text, text, uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.dismiss_undo_atomic(text, integer, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.merge_tables_atomic(text, integer, integer[], uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.unmerge_tables_atomic(text, integer, integer[], uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.transfer_table_atomic(text, integer, integer, uuid, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.release_paid_table_atomic(text, integer, text, uuid, text) TO service_role;
