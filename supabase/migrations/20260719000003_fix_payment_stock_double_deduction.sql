-- Prevent double stock deduction in process_order_payment.
-- deductStockForOrder (used by mark-ready) already writes order_consumption
-- logs, but process_order_payment re-deducted unconditionally -> stock was
-- removed twice for any order marked ready before payment.
-- Add the same idempotency guard: skip the deduction loop (and reuse the
-- already-recorded COGS) when order_consumption logs already exist.
...
BEGIN
  ...
  -- Idempotency guard: stock may already have been deducted when the kitchen
  -- marked the order ready (deductStockForOrder). Reuse recorded COGS instead
  -- of writing a second batch of order_consumption logs.
  SELECT EXISTS (
  ...
  ELSE
    FOR v_ingredient IN
      SELECT oi.product_id, oi.quantity,
             r.ingredient_id, r.quantity AS ingred_qty,
  ...
  UPDATE table_floors SET
    status = 'empty', total_amount = 0, guest_count = NULL, order_count = 0
  WHERE table_number = v_order.table_number;

  INSERT INTO audit_log (table_name, record_id, action, old_data, new_data, changed_by)
  VALUES (
    'orders', p_order_id, 'payment',
    jsonb_build_object('status', v_order.status),
    jsonb_build_object('status', 'paid', 'amount', v_total_paid, 'cogs', v_cogs, 'profit', v_profit, 'method', p_payment_method, 'discount_type', p_discount_type, 'discount_amount', p_discount_amount, 'tip', p_tip_amount, 'total_paid', v_total_paid),
    p_performed_by
  );
