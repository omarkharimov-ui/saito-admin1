...
// GC 1.5 (2026-09-20, map §8 "reload"): top-up a LIVE gift card.
// Only 'active' cards can be loaded (spent card + money = refund semantics);
// lazy expiry guard (expires_at < now) mirrors frozen gift_card_redeem (000005).
// Body: { amount: number, reason?: string }.
// SECURITY CHAIN (Rule 10): staff session -> requireAuth -> GC role gate ->
// service_role key -> gift_card_load() RPC (FOR UPDATE atomic, ledger 'load'
// entry, log_audit, balance invariant preserved).
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    const res = await fetch(`${url}/rest/v1/rpc/gift_card_load`, {
      method: 'POST',
      headers: {
        'apikey': key,
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Load operation failed: ${t}` }, { status: 500 });
    }
