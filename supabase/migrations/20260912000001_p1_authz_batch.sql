-- =============================================================================
-- P-1 AUTHORIZATION BATCH (ratified 2026-09-12) — single clean migration
-- M1  contract freeze: has_permission(p_staff_id, p_permission) = permission SSOT
-- M2  FIX (variant b): DB-level location assertion on the financial write path
--     (complete_payment_atomic_v2 13->14 args, refund_with_inventory 9->10 args)
-- M3  retire 4 broken/dead permission RPCs (0 app consumers, pg_depend clean)
-- M5  owner role permissions seed (all 65 keys, idempotent)
-- M6  retire override tables (0 rows live, never consulted by the gate)
-- M4/M7 live on the app side (routes + .p1-gate.cjs), not in this file.
-- Rollback reference: supabase/migrations/_p1_rollback_20260912/
-- =============================================================================

-- ===== M2 helper: fail-closed location assertion ============================
-- Enforces (for a non-bypass actor):
--   (a) the target location is one the actor is ACTIVELY bound to
--   (b) OR the org has data in EXACTLY ONE location (documented
--       single-location-with-data fallback — same rule as
--       resolveWriteLocationContext step 2 in the app; keeps unbound staff
--       operable in a 1-location org and fails closed the moment a 2nd
--       location gains data)
-- Bypass: superadmin/owner roles (consistent with is_superadmin() used by RLS).
CREATE OR REPLACE FUNCTION public.p1_actor_allowed_at_location(p_actor_id uuid, p_location_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
AS $$
  SELECT EXISTS (
    ...
    WHERE s.id = p_actor_id
      AND (
        EXISTS (SELECT 1 FROM staff_locations sl
                WHERE sl.staff_id = p_actor_id
                  AND sl.location_id = p_location_id
                  AND sl.active = true)
        OR (SELECT r.name FROM roles r WHERE r.id = s.role_id) IN ('superadmin', 'owner')
        OR (
          (SELECT count(DISTINCT o.location_id) FROM orders o WHERE o.location_id IS NOT NULL) = 1
          AND EXISTS (SELECT 1 FROM orders o WHERE o.location_id = p_location_id)
        )
      )
  );
$$;

COMMENT ON FUNCTION public.p1_actor_allowed_at_location(uuid, uuid) IS
  'P-1 M2: location-scope assertion for financial writes (fail-closed). P1_AUTHZ_CONTRACT_FREEZE_2026-09-12.md §8/M2.';

-- ===== M2: complete_payment_atomic_v2 — location-asserted rewrite ===========
-- (original 13-arg body + the M2 assertion; identical otherwise)
...
-- ===== M3: retire broken/dead permission RPCs (0 app consumers; pg_depend clean) =====
DROP FUNCTION IF EXISTS public.has_permission_v2(uuid, character varying, uuid);
DROP FUNCTION IF EXISTS public.check_permission(uuid, text);
DROP FUNCTION IF EXISTS public.get_effective_permissions(uuid);
DROP FUNCTION IF EXISTS public.get_effective_permissions_v2(uuid, uuid, uuid);

-- ===== M5: owner role permissions seed (all 65 keys; idempotent) =====
-- owner is the top business role: full financial + operational scope
-- (matches the superadmin grant set; owner already bypasses location via
-- p1_actor_allowed_at_location).
INSERT INTO role_permissions (role_id, permission_key)
  SELECT (SELECT id FROM roles WHERE name = 'owner'), p.key
  FROM permissions p
  WHERE NOT EXISTS (
...
-- ===== M6: retire override tables (0 rows live; gate never consulted them) =====
-- Note: check_permission()/get_effective_permissions() read staff_permission_overrides,
-- but both are dropped in M3 (above) first, so no dependent function remains.
DROP TABLE IF EXISTS public.staff_permission_overrides CASCADE;
DROP TABLE IF EXISTS public.location_permission_overrides CASCADE;

-- ===== M1: contract marker (no code; the SSOT is has_permission(uuid, text)) =====
COMMENT ON FUNCTION public.has_permission(uuid, text) IS
  'P-1 M1 (frozen 2026-09-12): THE permission SSOT — role_permissions x staff.is_active. '
  'Location scope is enforced at the financial-write layer (p1_actor_allowed_at_location '
  'inside complete_payment_atomic_v2 / refund_with_inventory), NOT here. '
  'See P1_AUTHZ_CONTRACT_FREEZE_2026-09-12.md.';
