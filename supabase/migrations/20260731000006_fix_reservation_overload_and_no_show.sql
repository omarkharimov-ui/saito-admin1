-- Migration 6: fix reservation update overload ambiguity + auto_no_show cast
--  * The date/integer[] overload of update_reservation_atomic made the app's
--    call (p_date text + p_table_ids array) ambiguous (PGRST203). Drop it; the
--    text/jsonb overload is the one the reservations route calls.
--  * auto_no_show_v2: reservations.table_ids is a real integer[] column now,
--    drop the integer[]::jsonb cast from the rewrite.

DROP FUNCTION IF EXISTS public.update_reservation_atomic(
...
-- activate_table_atomic: orders.total_amount is NOT NULL, was omitted
