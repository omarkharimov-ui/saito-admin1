CREATE FUNCTION public.cancel_delivery_order (
  p_order_id     uuid,
...
BEGIN
  ...
  PERFORM log_order_event(
    ...
    jsonb_build_object('status', v_order.status, 'delivery_status', v_order.delivery_status),
...
GRANT ALL ON FUNCTION public.cancel_delivery_order(uuid, text, uuid) TO anon;

GRANT ALL ON FUNCTION public.cancel_delivery_order(uuid, text, uuid) TO authenticated;

GRANT ALL ON FUNCTION public.cancel_delivery_order(uuid, text, uuid) TO service_role;