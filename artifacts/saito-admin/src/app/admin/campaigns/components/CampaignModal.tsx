  ...
  const formBody = (
    <form noValidate onSubmit={handleSubmit} className="w-full overflow-hidden flex flex-col">
          ...
          return (
            <React.Fragment key={title}>
              <button type="button" onClick={() => setStep(n)}
                ...
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${active ? 'bg-[var(--theme-accent)] text-black' : done ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-[var(--theme-surface-soft)] text-[var(--theme-text-muted)] border border-[var(--theme-border)]'}`}>
                  {done ? '✓' : n}
                </span>
      ...
      <div className="flex-1 px-4 md:px-6 py-2 space-y-5 overflow-y-auto" style={{ maxHeight: '62vh' }}>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {campaignTypes.map((ct) => (
                <button key={ct.id} type="button" onClick={() => {
                  ...
                  onFormChange({ ...form, type: newType, rules: [defaultRule], targets: [{ target_type: 'whole_order' }] });
                }}
                  className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${form.type === ct.id ? 'bg-[var(--theme-accent-soft)] border-[var(--theme-accent-border)]' : 'bg-[var(--theme-surface-soft)] border-[var(--theme-border)] hover:border-[var(--theme-border-strong)]'}`}>
                  <span className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${form.type === ct.id ? 'bg-[var(--theme-accent)] text-black' : 'bg-[var(--theme-surface)] text-[var(--theme-text-muted)]'}`}>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {['product', 'category', 'whole_order', 'combo'].map(targetType => (
            ...
            {(form.targets[0]?.target_type === 'product' || form.targets[0]?.target_type === 'category') && (
              <div className="space-y-3">
                ...
                {/* Category filter — only for products, single place */}
        ...
        {/* STEP 3 — PARAMETERS (only type-relevant fields) */}
        {step === 3 && (
          <div className="space-y-5">
            ...
            {/* Percentage — only for PERCENTAGE / HAPPY_HOUR */}
            {isPercentage && (
              <div className="space-y-2">
            ...
            {/* Happy Hour time — only for HAPPY_HOUR */}
            {isHappyHour && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
              ...
              {showAdvanced && (
                <div className="space-y-4 pt-2">
                  ...
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={form.stackable} onChange={(e) => onFormChange({ ...form, stackable: e.target.checked })} className="w-4 h-4 rounded border-[var(--theme-border)] bg-[var(--theme-surface-soft)] text-[var(--theme-accent)]" />
                    <span className="text-[12px] text-[var(--theme-text-secondary)]">Stackable</span>
                  </label>
                  {/* Coupon */}
                  <div className="space-y-2">
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={form.requires_coupon} onChange={(e) => onFormChange({ ...form, requires_coupon: e.target.checked })} className="w-4 h-4 rounded border-[var(--theme-border)] bg-[var(--theme-surface-soft)] text-[var(--theme-accent)]" />
                  ...
                  {/* Auto apply */}
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={form.auto_apply} onChange={(e) => onFormChange({ ...form, auto_apply: e.target.checked })} className="w-4 h-4 rounded border-[var(--theme-border)] bg-[var(--theme-surface-soft)] text-[var(--theme-accent)]" />
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="fixed bottom-0 inset-x-0 px-4 pb-7 pt-3 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/95 to-transparent z-10 flex gap-2">
              {step > 1 && (
              ...
              {step < 4 ? (
                <button type="button" onClick={() => setStep(s => s + 1)} className="flex-[2] py-3.5 rounded-xl font-bold tracking-[0.1em] uppercase transition-all flex items-center justify-center gap-2 bg-[var(--theme-accent)] text-black border border-[var(--theme-accent-border)]">
                  İrəli
                </button>
              ) : (
                <button type="button" onClick={onSubmit as any} disabled={isSubmitting} className="flex-[2] py-3.5 rounded-xl font-bold tracking-[0.1em] uppercase transition-all flex items-center justify-center gap-2 disabled:opacity-40 bg-[var(--theme-accent)] text-black border border-[var(--theme-accent-border)]">
          ...
          <div className="fixed inset-0 z-[120] hidden md:flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="sticky bottom-0 px-5 py-3.5 bg-card border-t border-white/[0.06] flex gap-2">
                ...
                {step < 4 ? (
                  <button type="button" onClick={() => setStep(s => s + 1)} className="flex-1 py-3 rounded-xl font-bold tracking-[0.1em] uppercase transition-all flex items-center justify-center gap-2 bg-[var(--theme-accent)] text-black border border-[var(--theme-accent-border)]">
                    İrəli
                  </button>
                ) : (
                  <button type="button" onClick={onSubmit as any} disabled={isSubmitting} className="flex-1 py-3 rounded-xl font-bold tracking-[0.1em] uppercase transition-all flex items-center justify-center gap-2 disabled:opacity-40 bg-[var(--theme-accent)] text-black border border-[var(--theme-accent-border)]">
                    {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                    {campaign ? t('edit_campaign').toUpperCase() : t('new_campaign').toUpperCase()}
                  </button>
                )}
