        ...
        <>
          ...
          <div className="fixed inset-0 z-[110] hidden md:flex items-center justify-center p-4 xl:p-10">
            ...
            <motion.div
              ...
              className={`relative w-full max-w-[860px] bg-card/98 backdrop-blur-xl border rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden ${lightMode ? 'border-gray-200 shadow-black/8' : 'border-white/10 shadow-black/60'}`}
            >
              <button onClick={closeWithReset} className={`absolute top-5 right-5 z-10 w-10 h-10 rounded-full bg-white/8 hover:bg-white/15 flex items-center justify-center transition-all ${lightMode ? 'text-gray-500 hover:text-gray-900' : 'text-white/50 hover:text-white'}`}><X size={18} /></button>
              <div className="flex flex-col flex-1 min-h-0">
                <div className="px-8 lg:px-12 pt-7 pb-0 shrink-0">
                  <h2 className={`text-3xl font-serif font-bold mb-1 tracking-tight ${lightMode ? 'text-gray-900' : 'text-white'}`}>{editingProduct ? t('edit_product') : t('new_product')}</h2>
              ...
              <div>
                <p className={`text-[9px] uppercase tracking-[0.3em] font-bold mb-3 flex items-center gap-2 ${lightMode ? 'text-gray-300' : 'text-white/20'}`}>
                  <span className={`w-4 h-px ${lightMode ? 'bg-gray-200' : 'bg-white/10'}`} />
                  {t('basic_info_section')}
                  <span className={`flex-1 h-px ${lightMode ? 'bg-gray-100' : 'bg-white/5'}`} />
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="col-span-2 space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full border rounded-xl px-4 py-2.5 text-sm outline-none transition-transform duration-300 ${lightMode ? 'bg-gray-100 text-gray-900 placeholder:text-gray-400' : 'bg-white/[0.07] text-white placeholder:text-white/30'}${nameError ? 'border-red-500/70 focus:border-red-400' : 'border-white/[0.12] focus:border-white/35'}`}
                      />
                      <AnimatePresence>
                        {ghostMain?.name && !productForm.name && (
                          ...
                          >
                            {ghostMain.name.split(' ').map((word, i) => (
                              <motion.span key={i} variants={{ hidden: { opacity: 0, x: -6 }, visible: { opacity: 1, x: 0 } }} transition={{ duration: 0.22 }} className={`text-sm italic shrink-0 ${lightMode ? 'text-gray-400' : 'text-white/30'}`}>
                                {word}
                              </motion.span>
                            ))}
                          </motion.div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={`w-full border rounded-xl px-4 py-2.5 text-sm outline-none transition-all duration-300 ${lightMode ? 'placeholder:text-gray-400' : 'placeholder:text-white/30'}${
                        productForm.variants.length > 0
              ...
              <div>
                    ...
                    <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.97 }} transition={{ duration: 0.28, ease: [0.4, 0, 0.2, 1] }} className="mb-3">
                      {(uploadingImage || visionLoadingMain) ? (
                        ...
                        <motion.div ref={orbMainRef} animate={{ y: [0, -2, 0], boxShadow: ['0 0 0px rgba(212,175,55,0)', '0 0 18px rgba(212,175,55,0.18)', '0 0 0px rgba(212,175,55,0)'] }} transition={{ duration: 2.8, repeat: Infinity, ease: 'easeInOut' }} className={`group relative flex items-center gap-3 px-4 py-3 rounded-2xl border overflow-hidden cursor-default ${lightMode ? 'border-gray-200' : 'border-white/10'}`} style={{ background: lightMode ? '#f3f4f6' : 'rgba(255,255,255,0.03)', backdropFilter: 'blur(12px)' }}>
                          ...
                          <button type="button" onClick={dismissGhostMain} className={`shrink-0 w-7 h-7 flex items-center justify-center rounded-xl hover:text-white/55 transition-all ${lightMode ? 'text-gray-300 hover:bg-gray-100' : 'text-white/20 hover:bg-white/[0.05]'}`}><X size={13} /></button>
                        </motion.div>
                      )}
                    </motion.div>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-3">
                      ...
                      {uploadingImage ? (
                        ...
                        <>
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-transform duration-300 flex items-center justify-center">
                            <Upload size={18} className={`opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${lightMode ? 'text-gray-900' : 'text-white'}`} />
                          </div>
                        </>
                      ) : (
                  ...
                  <div className="space-y-2.5">
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <textarea value={productForm.description}
                          ...
                          className={`w-full border rounded-xl px-3.5 py-2 pb-5 text-sm placeholder:text-white/15 focus:border-white/30 outline-none transition-transform duration-300 h-[72px] resize-y min-h-[52px] max-h-[160px] ${lightMode ? 'bg-gray-50 border-gray-200 text-gray-900' : 'bg-white/[0.03] border-white/10 text-white'}`}
                        />
                        <AnimatePresence>
                          {ghostMain?.description && !productForm.description && (
                            <motion.div initial="hidden" animate="visible" exit={{ opacity: 0, y: 4, transition: { duration: 0.2 } }} variants={{ visible: { transition: { staggerChildren: 0.03 } } }} className="absolute inset-0 px-3.5 py-2 pb-5 pointer-events-none overflow-hidden flex flex-wrap content-start gap-x-[0.28em] gap-y-0">
                              {ghostMain.description.split(' ').map((word, i) => (
                                <motion.span key={i} variants={{ hidden: { opacity: 0, x: -5 }, visible: { opacity: 1, x: 0 } }} transition={{ duration: 0.2 }} className={`text-sm italic leading-snug shrink-0 ${lightMode ? 'text-gray-400' : 'text-white/30'}`}>{word}</motion.span>
                              ))}
                            </motion.div>
                          )}
                        </AnimatePresence>
                    ...
                    <div className="space-y-1">
                      <label className={`text-[10px] uppercase tracking-widest ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>{t('ingredients_label')} {t('ingredients_hint')}</label>
                      <div className="relative">
                        <textarea value={productForm.ingredients}
                          ...
                          className={`w-full border rounded-xl px-3.5 py-2 text-sm placeholder:text-white/15 focus:border-white/30 outline-none transition-transform duration-300 h-[38px] resize-none ${lightMode ? 'bg-gray-50 border-gray-200 text-gray-900' : 'bg-white/[0.03] border-white/10 text-white'}`} />
                        <AnimatePresence>
                  ...
                  {productForm.is_ready_product && (
                    ...
                    >
                      <div className="flex items-center gap-2 pl-1">
                        <span className={`text-[10px] uppercase tracking-wider ${lightMode ? 'text-gray-400' : 'text-white/30'}`}>Anbar:</span>
                        <select
                          value={productForm.direct_ingredient_id}
                          onChange={(e) => onFormChange({ ...productForm, direct_ingredient_id: e.target.value })}
                          className={`flex-1 max-w-xs border rounded-lg px-3 py-1.5 text-xs outline-none ${lightMode ? 'bg-gray-100 border-gray-200 text-gray-900 focus:border-gray-400' : 'bg-white/[0.05] border-white/10 text-white focus:border-white/25'}`}
                        >
                          <option value="" className={lightMode ? 'bg-gray-100 text-gray-400' : 'bg-[#1a1a1a] text-white/40'}>Xəmmal seçin...</option>
                          {ingredients.map(i => (
                            <option key={i.id} value={i.id} className={lightMode ? 'bg-gray-100' : 'bg-[#1a1a1a]'}>{i.name} ({i.unit})</option>
                          ))}
                        </select>
                      </div>
                    </motion.div>
              ...
              <div>
                ...
                <div className={`px-8 lg:px-12 py-4 bg-card/80 backdrop-blur-xl border-t rounded-b-2xl flex items-center gap-4 shrink-0 ${lightMode ? 'border-gray-100' : 'border-white/5'}`}>

                  <button type="button" onClick={closeWithReset} className={`px-8 py-3.5 rounded-xl border text-[10px] font-bold tracking-wide uppercase whitespace-nowrap transition-all ${lightMode ? 'bg-gray-100 text-gray-400 border-gray-300 hover:bg-gray-200 hover:text-gray-900' : 'bg-white/[0.05] text-white/40 border-white/[0.12] hover:bg-white/10 hover:text-white'}`}>
                    {t('cancel') || 'LƏĞV ET'}
                  </button>
          ...
          <motion.div
            ...
            className={`fixed inset-0 z-[110] md:hidden flex flex-col ${lightMode ? 'bg-white' : 'bg-[#0a0a0a]'}`}
          >
            {/* Mobile Header */}
            ...
            <form id="product-modal-form-mobile" noValidate onSubmit={onSubmit}
              ...
              <div className="space-y-1.5">
                ...
                <input type="number" step="0.01" min="0.01"
                  ...
                  readOnly={productForm.variants.length > 0}
                  onChange={(e) => { if (productForm.variants.length > 0) return; onFormChange({ ...productForm, price: e.target.value }); if (priceError) onPriceErrorChange(false); }}
                  className={`w-full border rounded-xl px-4 py-3 text-sm outline-none transition-all ${lightMode ? 'placeholder:text-gray-400' : 'placeholder:text-white/30'}${productForm.variants.length > 0 ? 'bg-white/[0.03] border-white/[0.06] text-gold/80 cursor-default' : priceError ? 'bg-white/[0.07] border-red-500/70 focus:border-red-400 text-white' : 'bg-white/[0.07] border-white/[0.12] focus:border-white/35 text-white'}`}
                />
                {priceError && <p className="text-[10px] text-red-400 mt-1">{t('price_required')}</p>}
              </div>
              ...
              <div className="space-y-3">
                    ...
                    <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.97 }} className="mb-3">
                      {(uploadingImage || visionLoadingMain) ? (
                        ...
                        <motion.div ref={orbMainRef} animate={{ y: [0, -2, 0], boxShadow: ['0 0 0px rgba(212,175,55,0)', '0 0 18px rgba(212,175,55,0.18)', '0 0 0px rgba(212,175,55,0)'] }} transition={{ duration: 2.8, repeat: Infinity, ease: 'easeInOut' }} className={`group relative flex items-center gap-3 px-4 py-3 rounded-2xl border overflow-hidden ${lightMode ? 'border-gray-200' : 'border-white/10'}`} style={{ background: lightMode ? '#f3f4f6' : 'rgba(255,255,255,0.03)', backdropFilter: 'blur(12px)' }}>
                          ...
                          <motion.button type="button" whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }} onClick={applyGhostMain} className={`shrink-0 relative overflow-hidden flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[9px] font-bold uppercase tracking-wider ${lightMode ? 'border-gray-300 text-gray-600' : 'border-white/20 text-white/70'}`}>
                            <motion.span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -skew-x-12 pointer-events-none" animate={{ x: ['-130%', '230%'] }} transition={{ duration: 1.8, repeat: Infinity, repeatDelay: 1.2, ease: 'easeInOut' }} />
                            <Wand2 size={9} /> {t('ai_hint_apply')}
                          </motion.button>
                          <button type="button" onClick={dismissGhostMain} className={`shrink-0 w-7 h-7 flex items-center justify-center rounded-xl hover:text-white/55 transition-all ${lightMode ? 'text-gray-300 hover:bg-gray-100' : 'text-white/20 hover:bg-white/[0.05]'}`}><X size={13} /></button>
                ...
                <div className="grid grid-cols-[auto_1fr] gap-3">
                  <div className="relative group">
                    ...
                    <label htmlFor="product-image-upload-mobile" className={`w-24 h-24 rounded-xl border-2 border-dashed flex items-center justify-center hover:border-white/30 transition-all cursor-pointer overflow-hidden relative ${lightMode ? 'bg-gray-50 border-gray-200 hover:bg-gray-100' : 'bg-white/[0.02] border-white/10 hover:bg-white/[0.05]'}`}>
                      {uploadingImage ? (
                        <svg width="32" height="32" viewBox="0 0 32 32" className="animate-spin">
                          <circle cx="16" cy="16" r="12" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="2.5"/>
                          <circle cx="16" cy="16" r="12" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="28 50" style={{filter:'drop-shadow(0 0 6px rgba(255,255,255,0.3))'}}/>
                  ...
                  <div className="space-y-2.5">
                    <div className="space-y-1">
                      ...
                      <textarea value={productForm.description}
                        onChange={(e) => { if (e.target.value.length <= 150) onFormChange({ ...productForm, description: e.target.value }); }}
                        placeholder={ghostMain?.description ? '' : t('description_placeholder')}
                        className={`w-full border rounded-xl px-3.5 py-2 pb-5 text-sm placeholder:text-white/15 focus:border-white/30 outline-none transition-all h-[72px] resize-y min-h-[52px] max-h-[160px] ${lightMode ? 'bg-gray-50 border-gray-200 text-gray-900' : 'bg-white/[0.03] border-white/10 text-white'}`}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className={`text-[10px] uppercase tracking-widest ${lightMode ? 'text-gray-400' : 'text-white/40'}`}>{t('ingredients_label')} {t('ingredients_hint')}</label>
                      <textarea value={productForm.ingredients}
                        ...
                        className={`w-full border rounded-xl px-3.5 py-2 text-sm placeholder:text-white/15 focus:border-white/30 outline-none transition-all h-[38px] resize-none ${lightMode ? 'bg-gray-50 border-gray-200 text-gray-900' : 'bg-white/[0.03] border-white/10 text-white'}`}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="z-[70] px-0 py-6 flex items-center gap-3">
                <button type="button" onClick={closeWithReset} className={`px-6 py-3.5 rounded-xl border text-[10px] font-bold tracking-wide uppercase whitespace-nowrap transition-all ${lightMode ? 'bg-gray-100 text-gray-400 border-gray-300' : 'bg-white/[0.05] text-white/40 border-white/[0.10]'}`}>
                  {t('cancel') || 'LƏĞV ET'}
                </button>
