...
const GeneralTab = ({ initialData }: { initialData?: Record<string, any> | null }) => {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <form noValidate onSubmit={save} className="space-y-6 rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-surface-muted)] p-6 sm:p-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <p className="text-[9px] uppercase tracking-[0.18em] text-[var(--theme-text-muted)] mb-1.5">{t('gen_open_time')}</p>
                <input
                  type="time"
                  value={openTime}
              ...
              <ChevronRight size={16} className="text-[var(--theme-text-muted)] mt-5 flex-shrink-0" />
              <div className="flex-1">
                <p className="text-[9px] uppercase tracking-[0.18em] text-[var(--theme-text-muted)] mb-1.5">{t('gen_close_time')}</p>
                <input
                  type="time"
                  value={closeTime}
        ...
        <div className="flex justify-end pt-2">
          <button type="submit" disabled={updating || !isDirty}
            className={`${saveButtonCls} bg-[var(--theme-surface)] text-[var(--theme-text)] border border-[var(--theme-border)] ${!isDirty && !updating ? 'opacity-40 pointer-events-none' : 'hover:bg-[var(--theme-panel)]'}`}>
