...
export default function MobileTopBar({ role }: { role: 'admin' | 'superadmin' | null }) {
  ...
  const langMenu =
    langOpen && mounted ? (
      <div
        ref={langRef}
        className="fixed z-[200] flex flex-col gap-1 min-w-[52px] rounded-xl border border-white/10 bg-[#0c0c0c] p-1 shadow-xl"
      ...
      >
        {LANGS.filter((l) => l.code !== language).map((lang) => (
          <button
            ...
            className="h-8 px-3 rounded-lg bg-white/[0.04] border border-white/[0.08] text-[10px] font-bold text-white/60 hover:text-gold hover:border-gold/30 transition-colors"
          >
            {lang.label}
          </button>
        ))}
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-white/[0.06] bg-[#0a0a0a] px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] lg:hidden isolate">
        ...
        <div className="relative">
          ...
          {showDropdown && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-[min(18rem,calc(100vw-1.5rem))] max-h-80 overflow-y-auto rounded-xl border border-white/10 bg-[#0a0a0a] shadow-2xl z-50">
      ...
      {mounted && langMenu ? createPortal(langMenu, document.body) : null}
    </header>
  );
}
