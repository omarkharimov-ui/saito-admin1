-- ============================================================================
-- 20260911000042 — L5 (decision A, ratified 2026-09-12): transition_order_atomic
--                  floor-sync reconciliation — table stays occupied
--
-- PROVEN REGRESSION (L1-2, K_FOUNDATION_CURRENT_STATE.md § LIFECYCLE RECONCILIATION):
--   table_floors live data uses exactly 3 states: empty / occupied / reserved
--   (seating writes 'occupied' by raw UPDATE). But transition_order_atomic's
--   6c block mapped order status -> the 13-STATE table registry
--   (in_kitchen/ready/dining/bill_requested/payment_pending) via
--   transition_table_status, and the registry has NO 'occupied->*' edge except
--   'occupied->merged'. Every intermediate floor-sync threw INVALID_TRANSITION,
--   silently caught -> audit_logs action='status_change_failed' (LIVE: 39 rows:
--   occupied->empty x24, occupied->in_kitchen x13, occupied->dining x1,
--   ready->dining x1), and 3 tables were left permanently stuck 'occupied'
--   (8, 401, 402 — repaired by migration 043).
--
-- RATIFIED FIX (decision A; the critical principle from the user: do NOT add
-- 'occupied -> in_kitchen/ready/...' edges to the F registry — the proven data
-- model is 4 separate lifecycles that derive/synchronize, not one state machine):
--   * The table lifecycle stays exactly empty / occupied / reserved.
--   * Active meal states (confirmed/in_kitchen/partially_ready/ready/served/
--     payment_pending/paid) keep the table OCCUPIED. Kitchen + service progress
--     live in orders.kitchen_status / order_items.kitchen_status; the floor's
--     kitchen chip is driven by table.kitchen_status (separate column, kept in
--     sync by its own trigger) — NOT by table.status.
--   * PAID + OCCUPIED is VALID (customer still sitting). Payment NEVER auto-
--     empties the table.
--   * Only a terminal cancellation frees the table — and only when no other
--     open order remains on that (table, location) — via a guarded raw UPDATE
--     (table_release_guard BEFORE trigger re-validates pointer/open-orders).
--   * 'closed' no longer auto-transitions the table (it never actually worked —
--     'occupied' has no '->cleaning' edge; the 39 failures include the attempts).
--     The canonical release is release_paid_table_atomic (explicit UI action,
--     customer departure) which owns the table->p_final_status step.
--
-- UNCHANGED (frozen O contract preserved): order status transition validation
-- (registry), permission + location-scope authorization (authorize), manager
-- override gate, the order UPDATE, kitchen_status sync (6a), kitchen_schedule
-- parity (6b), audit/operation/outbox emission (7-8), signature incl. param
-- defaults, SECURITY DEFINER, search_path, ACL (postgres + service_role only).
-- ============================================================================
...
-- ACL: unchanged (postgres + service_role only; token-first via p_token)
REVOKE ALL ON FUNCTION public.transition_order_atomic(text,uuid,text,text,jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.transition_order_atomic(text,uuid,text,text,jsonb) FROM authenticated;
REVOKE ALL ON FUNCTION public.transition_order_atomic(text,uuid,text,text,jsonb) FROM anon;
GRANT  EXECUTE ON FUNCTION public.transition_order_atomic(text,uuid,text,text,jsonb) TO service_role;

-- ---- fail-safe: shape + privilege ----
DO $$
BEGIN
  IF (SELECT count(*) FROM pg_proc WHERE pronamespace='public'::regnamespace
      AND proname='transition_order_atomic') <> 1 THEN
    RAISE EXCEPTION 'L5 FAIL-SAFE: transition_order_atomic must have exactly 1 overload';
  END IF;
  IF NOT has_function_privilege('service_role','public.transition_order_atomic(text,uuid,text,text,jsonb)','EXECUTE')
     OR has_function_privilege('authenticated','public.transition_order_atomic(text,uuid,text,text,jsonb)','EXECUTE')
     OR has_function_privilege('anon','public.transition_order_atomic(text,uuid,text,text,jsonb)','EXECUTE') THEN
    RAISE EXCEPTION 'L5 FAIL-SAFE: transition_order_atomic ACL must be service_role-only';
  END IF;
  -- 6c must no longer churn the 13-state registry (no v_target_table_status,
  -- no transition_table_status call), and must keep the occupied invariant.
  IF EXISTS (SELECT 1 FROM pg_proc WHERE pronamespace='public'::regnamespace
             AND proname='transition_order_atomic'
             AND (prosrc LIKE '%v_target_table_status%' OR prosrc LIKE '%transition_table_status%')) THEN
    RAISE EXCEPTION 'L5 FAIL-SAFE: 6c still references the table registry churn path';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE pronamespace='public'::regnamespace
                 AND proname='transition_order_atomic'
                 AND prosrc LIKE '%6c. floor sync (L5/A, 2026-09-12, ratified)%') THEN
    RAISE EXCEPTION 'L5 FAIL-SAFE: ratified 6c block missing';
  END IF;
END $$;
