-- ─────────────────────────────────────────────────────────────────────
-- Migration 20260908000005
-- D-6 + D-7: Payment Idempotency + Overpay Guard + Payment Ledger
-- ─────────────────────────────────────────────────────────────────────
-- Fix scope: complete_payment_atomic_v2 ONLY (live POS pay path).
-- GUARD ORDER (important, race-safe):
--   1. FOR UPDATE (serialize writers)
--   2. idempotency dedupe (same key → return prior, no charge)
--   3. parse payments (split non-refund vs refund)
--   4. refund guard   (refund ≤ net paid)
--   5. already-paid   (non-refund on fully-paid order → reject)
--   6. overpay guard  (non-refund ≤ remaining)
--   7. compute, ledger insert, order update, store key, audit
--   (5 before 6 ⇒ a fully-paid order + fresh key yields ORDER_ALREADY_PAID
--    (route maps to HTTP 409) rather than a generic overpay 500; refunds on
--    paid orders are still allowed since v_non_refund_total = 0.)
--
-- Does NOT change: state machine, RLS, auth/validate_actor, merge/transfer/
--   pointer/reservation contracts, v1 (dead code), refund_with_inventory,
--   validate_payment_order_balance trigger, or any other trigger.
-- ─────────────────────────────────────────────────────────────────────

-- Parameter list matches the LIVE signature EXACTLY (all defaults) so
-- CREATE OR REPLACE does not add/remove defaults.
